...
next:
  ...
  if (p == InternalEntry::Null()) {
    ...
    assert(leaf_addr == GlobalAddress::Null()); // 加上这些assert或printf语句，性能降低10%或者发生core dumped。2023年11月22日22:28:31。开始寻找原因
    // if(leaf_addr != GlobalAddress::Null()) printf("error!\n");
    bool res = out_of_place_write_leaf(k, v, depth, leaf_addr, get_partial(k, depth-1), p_ptr, p, node_ptr, cas_buffer, cxt, coro_id);
    assert(depth == 1);
    // if(depth != 1) printf("error!\n");
    // cas fail, retry
    if (!res) {
  ...
  if (p.is_leaf) {
    ...
    // printf("depth: %d, key: %d, key_in_leaf: %d\n", depth, key2int(k), key2int(_k));
...
write_leaf:
  // GlobalAddress p_ptr = root_ptr_ptr; 
  // auto leaf_buffer = (dsm->get_rbuf(coro_id)).get_leaf_buffer();
  // read_leaf(leaf_addr, leaf_buffer, sizeof(Leaf), p_ptr, false, cxt, coro_id);
#ifdef TREE_TEST_HOCL_HANDOVER
...
next:
  ...
  // 1. If we are at a NULL node, inject a leaf, 这里是否有问题，如果索引缓存是旧的？
  if (p == InternalEntry::Null()) {
    assert(from_cache == false);
    search_res = false;
    goto search_finish;
  ...
  if (p.is_leaf) {
    ...
    if (!is_valid) { //什么情况下是无效的？它的判断依据是叶子中的valid位以及rev_ptr是否符合预期
#ifdef TREE_ENABLE_CACHE
      // invalidate the old leaf entry cache
      if (from_cache) {
        index_cache->invalidate(entry_ptr_ptr, entry_ptr);
  ...
  if (depth == hdr.depth) { // 如果最近读取到的内部节点深度合法，再将其加入缓存，请注意，这里无论节点是否已经存在，都将合法的节点加入缓存队列，因此缓存有一定的LFU思想
    index_cache->add_to_cache(k, p_node, GADD(p.addr(), sizeof(GlobalAddress) + sizeof(Header)));
  }
#else
  UNUSED(type_correct);
  ...
  for (int i = 0; i < hdr.partial_len; ++ i) { //检查前缀
    if (get_partial(k, hdr.depth + i) != hdr.partial[i]) { //前缀不完全匹配，需要分裂
      search_res = false;
      goto search_finish;
    }
  }
  // 前缀完全匹配，则寻找是否有相同前缀的InternalEntry，如果有，则需要向下遍历，如果没有，则需要寻找空槽插入新叶子
  depth = hdr.depth + hdr.partial_len;

  // 3.3 try get the next internalEntry
  max_num = node_type_to_num(p.type()); // 注意，这里用的是p，p应该是最新的
