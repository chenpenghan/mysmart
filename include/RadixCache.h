...
private:
  ...
  CacheNode* cache_root; // 根，使用它来遍历，寻找索引节点
  tbb::concurrent_queue<CacheNode*>* node_queue; // 按照访问先后顺序存储索引节点，然后利用顺序选择驱逐哪些“旧”索引节点

  // GC
  tbb::concurrent_queue<CacheEntry*> cache_entry_gc;
  tbb::concurrent_queue<CacheHeader*> cache_hdr_gc;
  static const int safely_free_epoch = 256 * MAX_APP_THREAD * MAX_CORO_NUM;
